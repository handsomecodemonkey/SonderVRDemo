		...
		private void DrawKeyframeTimeline() {
			...
			if (PanoplyCore.scene != null) {
				...
				for (i=0; i<n; i++) {
					...
					if (i < (n - 1)) {
						GUI.Label( new Rect( timelineRect.x + (unitWidth * i) + 5.5f, timelineRect.y, unitWidth + 1, 22 ), "" );
					}
				...
				GUI.Label( new Rect( timelineRect.x - 1 + (unitWidth * scriptIndexProp.intValue) + 5.5f, timelineRect.y, 3, 22 ), "" );
				...
				for (i=0; i<n; i++) {
					...
					if ((stateScriptProp.GetArrayElementAtIndex( i ).stringValue != InstrumentState.HoldCommand) && (i != draggedKeyframeIndex)) {
						GUI.Label( new Rect( timelineRect.x + 1 + (unitWidth * i), timelineRect.y + 5.5f, 11, 11 ), "" );
					}
