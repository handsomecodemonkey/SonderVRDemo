	...
	public class PanelEditor: Editor {
		...
		private void DrawKeyframeTimelines() {
			...
			if (PanoplyCore.scene != null) {
				...
				for (i=0; i<n; i++) {
					...
					if (i < (n - 1)) {
						if (isEditingFrameProp.boolValue) {
							GUI.Label( new Rect( frameTimelineRect.x + (unitWidth * i) + 5.5f, frameTimelineRect.y, unitWidth + 1, 22 ), "" );
						}
						if (isEditingCameraProp.boolValue) {
							GUI.Label( new Rect( cameraTimelineRect.x + (unitWidth * i) + 5.5f, cameraTimelineRect.y, unitWidth + 1, 22 ), "" );
						}
						if (isEditingPassiveMotionProp.boolValue) {
							GUI.Label( new Rect( passiveMotionTimelineRect.x + (unitWidth * i) + 5.5f, passiveMotionTimelineRect.y, unitWidth + 1, 22 ), "" );
						}
					}
				...
				if (isEditingFrameProp.boolValue) {
					GUI.Label( new Rect( frameTimelineRect.x - 1 + (unitWidth * scriptIndexProp.intValue) + 5.5f, frameTimelineRect.y, 3, 22 ), "" );
				}
				if (isEditingCameraProp.boolValue) {
					GUI.Label( new Rect( cameraTimelineRect.x - 1 + (unitWidth * scriptIndexProp.intValue) + 5.5f, cameraTimelineRect.y, 3, 22 ), "" );
				}
				if (isEditingPassiveMotionProp.boolValue) {
					GUI.Label( new Rect( passiveMotionTimelineRect.x - 1 + (unitWidth * scriptIndexProp.intValue) + 5.5f, passiveMotionTimelineRect.y, 3, 22 ), "" );
				}
				...
				for (i=0; i<n; i++) {
					...
					if (isEditingFrameProp.boolValue && (frameStateScriptProp.GetArrayElementAtIndex( i ).stringValue != FrameState.HoldCommand) && ((i != draggedKeyframeIndex) || (draggedKeyframeComponent != PanelComponent.PanelFrame))) {
						GUI.Label( new Rect( frameTimelineRect.x + 1 + (unitWidth * i), frameTimelineRect.y + 5.5f, 11, 11 ), "" );
					}
					if (isEditingCameraProp.boolValue && (cameraStateScriptProp.GetArrayElementAtIndex( i ).stringValue != CameraState.HoldCommand) && ((i != draggedKeyframeIndex) || (draggedKeyframeComponent != PanelComponent.PanelCamera))) {
						GUI.Label( new Rect( cameraTimelineRect.x + 1 + (unitWidth * i), cameraTimelineRect.y + 5.5f, 11, 11 ), "" );
					}
					if (isEditingPassiveMotionProp.boolValue && (passiveMotionStateScriptProp.GetArrayElementAtIndex( i ).stringValue != PassiveMotionState.HoldCommand) && ((i != draggedKeyframeIndex) || (draggedKeyframeComponent != PanelComponent.PanelPassiveMotion))) {
						GUI.Label( new Rect( passiveMotionTimelineRect.x + 1 + (unitWidth * i), passiveMotionTimelineRect.y + 5.5f, 11, 11 ), "" );
					}
				...
				if (draggedKeyframeIndex != -1) {
					...
					case PanelComponent.PanelCamera:
						r = cameraTimelineRect;
						break;

					case PanelComponent.PanelPassiveMotion:
						r = passiveMotionTimelineRect;
						break;
		...
		private void KeyframeEditor(PanelComponent component) {
			...
			if (PanoplyCore.scene != null) {
				...
				GUI.enabled = !isOnFirstStep && !isOnLastStep;
		...
		public override void OnInspectorGUI() {
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField( "Global timeline", GUILayout.Width(122.0f) );
			if (PanoplyCore.scene != null) {
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField( "Home position", GUILayout.Width(120.0f) );
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField( "Layout grid", GUILayout.Width(120.0f) );
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.LabelField( "Intercept interactions", GUILayout.Width(120.0f) );
			...
			if ( isEditingFrameProp.boolValue && (PanoplyCore.panoplyRenderer != null)) {
				...
				Rect ri = new Rect( r.x + gridMargin, r.y + gridMargin, r.width - ( gridMargin * 2 ), r.height - ( gridMargin * 2 ) );
				
				GUI.skin.label = currentGridSkin.textArea;
				
				int row;
				int col;
				for ( row = 0; row < rowsProp.intValue; row++ ) {
					for ( col = 0; col < columnsProp.intValue; col++ ) {
						// first and last columns
						if ( col == 0 ) {
							GUI.Label( new Rect( r.x, r.y + gridMargin + ( row * unitHeight ), gridMargin + 1.0f, unitHeight + 1.0f ), "" );
							GUI.Label( new Rect( r.x + gridMargin + ( unitWidth * columnsProp.intValue ), r.y + gridMargin + ( row * unitHeight ), gridMargin + 1.0f, unitHeight + 1.0f ), "" );
						}
						// first and last rows
						if ( row == 0 ) {
							GUI.Label( new Rect( r.x + gridMargin + ( col * unitWidth ), r.y, unitWidth + 1.0f, gridMargin + 1.0f ), "" );
							GUI.Label( new Rect( r.x + gridMargin + ( col * unitWidth ), r.y + gridMargin + ( unitHeight * rowsProp.intValue ), unitWidth + 1.0f, gridMargin + 1.0f ), "" );
						}
						GUI.Label( new Rect( r.x + gridMargin + ( col * unitWidth ), r.y + gridMargin + ( row * unitHeight ), unitWidth + 1.0f, unitHeight + 1.0f ), "" );
					}
				}
				EditorGUILayout.Space();
				...
				if ( frameState != null ) {
					...
					foreach ( Panel otherPanel in panels ) {
						if ( otherPanel != panel ) {
							...
							if ( ofs != null ) {
								...
								activeRect = new Rect( ri.x + (ri.width * ( otherPanel.left * ( 1.0f / otherPanel.columns ))), ri.y + (ri.height * ( otherPanel.top * ( 1.0f / otherPanel.rows ))), ri.width * ( otherPanel.width * ( 1.0f / otherPanel.columns )) + 1, ri.height * ( otherPanel.height * ( 1.0f / otherPanel.rows )) + 1 );
					...
					activeRect = new Rect( ri.x + (ri.width * ( leftProp.intValue * ( 1.0f / columnsProp.intValue ))), ri.y + (ri.height * ( topProp.intValue * ( 1.0f / rowsProp.intValue ))), ri.width * ( widthProp.intValue * ( 1.0f / columnsProp.intValue )) + 1, ri.height * ( heightProp.intValue * ( 1.0f / rowsProp.intValue )) + 1 );
					activeRect = ConstrainPanelRect( activeRect, r );
					...
					if ( Event.current.isMouse ) {
						...
						if (( pos.x >= r.x ) && ( pos.x <= ( r.x + r.width )) && ( pos.y >= r.y ) && ( pos.y <= ( r.y + r.height ))) {
							Vector2 unitPos = new Vector2( Mathf.Floor(( pos.x - ri.x ) / (float)unitWidth ), Mathf.Floor(( pos.y - ri.y ) / (float)unitHeight ) );
							switch ( Event.current.type ) {
					...
					/*if ( PanoplyCore.screenRect.width > 0 ) {
						//float hGridUnit = PanoplyCore.screenRect.width / columnsProp.intValue;
						//float vGridUnit = PanoplyCore.screenRect.height / rowsProp.intValue;
						topProp.intValue = ( int )Mathf.Round( ( frameState.FindPropertyRelative( "v" ).floatValue * .01f ) / ( 1.0f / rowsProp.intValue ) );
						leftProp.intValue = ( int )Mathf.Round( ( frameState.FindPropertyRelative( "h" ).floatValue * .01f ) / ( 1.0f / columnsProp.intValue ) );
					...
					if ( EditorGUI.EndChangeCheck() ) {
						/*if ( frameStateScriptProp.GetArrayElementAtIndex( scriptIndexProp.intValue ).stringValue == FrameState.HoldCommand ) {
							SetKeyframeStateTypeForComponent(StateType.Key, PanelComponent.PanelFrame);
						}*/
						PopulateFrameState( frameState );
					}
